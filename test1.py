...
def save_bot_config(config):
    save_data(BOT_CONFIG_FILE, config)
...
def is_user_joined_channel(bot, user_id, channel):
    try:
        return all(bot.get_chat_member(chat_id=channel, user_id=user_id).status in {'member', 'creator', 'administrator'} for canal in channel)
...
def balance(update: Update, context: CallbackContext) -> None:
    ...
    message = f"🚀 Account Information 🚀\n\n💰 Available Balance: {saldo_str} TRX\n👥 Earnings From Referrals: {refer_earn_str} TRX\n💸 Total Withdrawn: {total_withdrawn_str} TRX\n⚖️ Total Earnings: {ganancia_total_str} TRX\n"
